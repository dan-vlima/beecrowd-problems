...
var input: string = require('fs').readFileSync('1010-simple-calculate/dev/stdin', 'utf8');
...
const calcTotalPrice = (lines: string[]) => {
    ...
    const totalPrice: number = (unitsOfProduct1 * priceOfProduct1) + (unitsOfProduct2 * priceOfProduct2);
    return totalPrice;
};

const totalPrice: number = calcTotalPrice(lines);
